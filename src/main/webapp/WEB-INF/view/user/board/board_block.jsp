    ...
    $.ajax({
        ...
        success : function( data ) {
            ...
            data.boardList.forEach(function(board, index) {
				...
				if(board.thumb === "Y"){
					row += '<img src="/img/user/sample03.jpg" class="img-responsive" alt="" /></a>';
				} else {
					row += '<img src="/img/common/no_img.png" class="img-responsive" alt="" /></a>';
				}
                row += '</a>';
                row += '<div class="caption auto pt10">';
                row += '<div class="mb10">';
                row += '<span class="label label-xs label-danger">' + board.category_seq + '</span>';
                row += '</div>';
                row += '<a href="javascript:void(0);">';
                row += '<div class="h6 mb5 text-overflow">';
                row += '<strong>' + board.title + '</strong>';
                row += '</div>';
                row += '</a>';
                row += '<div class="text-muted small mb10">' + board.content + '</div>';
                row += '<hr class="thin">';
                row += '<div class="btn-group btn-group-justified mb5">';
                row += '<a href="#" class="btn btn-sm btn-link" role="button">';
                row += '<strong>조회</strong> <strong class="text-danger num-webfont ml5">' + board.view_cnt + '</strong>';
                row += '</a>';
                row += '<a href="#" class="btn btn-sm btn-link" role="button">';
                row += '<strong>좋아요</strong><strong class="text-danger num-webfont ml5">56</strong>';
                row += '</a>';
                row += '</div>';
                row += '</div>'; // closing for 'caption auto pt10'
                row += '</div>'; // closing for 'thumbnail'
