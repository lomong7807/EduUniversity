    ...
    $.ajax({
        ...
        success : function( data ) {
            ...
            data.boardList.forEach(function(board, index) {
                ...
                row += '<a href="/site/write/update/' + board.board_seq + '/' + ${config.config_seq}+'" class="thumbnail">';
				if(board.thumb === "Y"){
					row += '<img src="/img/user/sample03.jpg" class="img-responsive" alt="" /></a>';
				} else {
					row += '<img src="/img/common/no_img.png" class="img-responsive" alt="" /></a>';
				}
                row += '<div class="caption text-center mb0">';
                row += '<strong>' + board.title + '</strong> <span class="ico ico_new">새글</span>';
                row += '</div>';
                row += '<div class="text-center">' + board.write_id + '</div>';
                row += '</a>';
                row += '</div>';
