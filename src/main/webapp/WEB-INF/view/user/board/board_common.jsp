						...
						<fieldset>
							...
							<div id="customer_container">
								...
								<div class="category-wrap">
									...
									<input type="hidden" name="category_seq" id="category_seq"> <input type="hidden" id="config_seq">
								</div>
								<!-- //카테고리 타입1 -->
								...
								<div class="row mt15 mb10">
									...
									<div class="col-sm-6 text-right">
										...
										<form class="form-inline search-form" name="searchForm" action="#">
											<fieldset>
												<div class="form-group col-sm-3 md-pr10 pl0 md-pr10">
    ...
    $.ajax({
        ...
        success : function( data ) {
            ...
            data.boardList.forEach(function(board, index) {
                ...
                let row = '<tr class="odd pointer" data-href="/site/write/update/' + board.board_seq + "/"+${config.config_seq}+'">';
                    row += '<td class="hidden-xs">' + ((data.totalRecords - (data.currentPage - 1) * data.pageSize) - index) + '</td>';
                    row += '<td class="hidden-xs">' + board.title + '</td>';
                    if(board.is_file == 'Y'){
    	                row += '<td class="hidden-xs"><a href="#" class="ico ico_file" target="_blank" title="파일">파일</a></td>';
					} else {
	                    row += '<td class="hidden-xs"><a href="#" class="ico ico_file_non" target="_blank" title="파일">파일</a></td>';
					}
                    row += '<td class="hidden-xs">' + board.write_id + '</td>';
                    row += '<td class="hidden-xs">' + board.write_time + '</td>';
                    row += '<td class="hidden-xs">' + board.view_cnt + '</td>';
