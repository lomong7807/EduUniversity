...
//                     row += '<div class="hidden-xs">' + ((data.totalRecords - (data.currentPage - 1) * data.pageSize) - index) + '</div>';
					row += '<div class="col-sm-4 col-md-3">';
					...
					row += '<div class="caption text-center pt0">';
					row += '<a href="javascript:void(0);">';
					row += '<strong>' + board.title + '</strong>';
					row += '<span class="ico ico_new">새글</span></a>';
					row += '<div class="small">' + board.write_id + '</div>';
					row += '</div>'; // caption text-center pt0를 닫는 </div> 태그
					row += '</div>'; // col-sm-4 col-md-3를 닫는 </div> 태그
					row += '</div>'; // odd pointer를 닫는 </div> 태그
