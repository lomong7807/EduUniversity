										...
										<form class="form-inline search-form" name="searchForm" action="#">
											<fieldset>
												<div class="form-group">
													<label for="searchCate" class="sr-only">검색 옵션</label>
													<select id="searchCate" name="searchCate" class="form-control pr20">
														<option value="title">제목</option>
														<option value="content">내용</option>
													</select>
												</div>
												<div class="input-group">
													<label for="keywordInput" class="sr-only"></label>
													<input type="text" id="keywordInput" class="form-control" name="keyword" value="${keyword}" placeholder="키워드 입력">
													<span class="input-group-btn">
														<button type="button" class="btn btn-default" onclick="javascript:searchKey();">검색</button>
													</span>
												</div>
											</fieldset>
										</form>
    ...
    $.ajax({
        ...
        success : function( data ) {
            ...
            data.boardList.forEach(function(board, index) {
                let row = '<div class="odd pointer text-left">';
                row +='<div class="row" style="width: 100%">';
                row +='<div class="col-sm-4 col-lg-3">';
                row +='<a href="/site/write/update/' + board.board_seq + '/' + ${config.config_seq}+'">';
				if(board.thumb === "Y"){
					row += '<img src="/img/user/sample03.jpg" class="thumbnail thumbnail-non mb10 img-responsive" alt="" /></a>';
				} else {
					row += '<img src="/img/common/no_img.png" class="thumbnail thumbnail-non mb10 img-responsive" alt="" /></a>';
				}
                row +='	</a>';
               	row +='</div>';
         		row +='<div class="col-sm-8 col-lg-9">';
      			row +='<div class="caption auto pt0 mb10">';
    			row +='<a href="javascript:void(0);" class="h5  mb10">';
    			row +='<strong>'+board.title+'</strong>';
    			row +='<span class="ico ico_new">새글</span>';
    			row +='</a>';
    			row +='<div class="text-muted">';
    			row +='<span class="post-date num-webfont">';
    			row +='<i class="fa fa-calendar"></i>'+board.write_time+'';
    			row +='</span>';
    			row +=''+board.write_id+'';
    			row +='</div>';
    			row +='</div>';
    			row +='<p class="text-muted">'+board.content+'</p>';
    			row +='</div>';
    			row +='</div>';
    			row +='<hr/>';
