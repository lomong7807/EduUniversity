							...
							<fieldset>
												...
												<div class="row">
													...
													<div class="col-sm-5">
														<input id="keywordInput" type="text" class="form-control"
															name="keyword" value="" placeholder="Keyword">
													</div>
													<div class="col-sm-2">
														<button id="keywordSBtn" type="button"
															class="btn btn-block btn-default"
															onclick="javascript:searchBtnClicked();">
								...
								<div name="form" method="post" action="menu_proc.php">
									...
									<div class="row pull-right">
										<button id="rootMenus" href="#"
											class="btn btn-mini btn-warning" onclick="goRootMenu()">
											<i class="fa fa-folder-open"></i> 상위메뉴
										</button>
										<a href="#" class="btn btn-mini btn-default"
											onclick="deleteMenus();">취소하기 </a> <a href="#"
											class="btn btn-mini btn-primary"
											onclick="javascript:location.href='/admin/adminMenuRegister';">
											등록하기</a>
...
function searchKey(page = 1, pMenuSeq){
    ...
    $.ajax({
		...
		console.log(data);
            ...
            tableBody.empty();
            $('#smallWrapper').empty();
			data.adminMenuList.forEach(function(menu, index) {
                ...
                row += '</tr>';
             	...
             	// 작은 화면 목록 추가
                let srow = '<td class="text-left hidden-sm hidden-md hidden-lg">';
                srow += '<div class="hidden-sm hidden-md hidden-lg">';
                srow += '<ul class="list-icon mb5">';
                srow += '<li><span class="text-info">메뉴명 </span> : ' + menu.title + '</li>';
                srow += '<li><span class="text-info">메뉴단계 </span> : ' + menu.depth + '</li>';
                srow += '<li><span class="text-info">정렬번호 </span> : ' + menu.order_num + '</li>';
                srow += '<li><span class="text-info">접근레벨 </span> : ' + menu.auth_level + '</li>';
                if(menu.p_menu_seq == 0){
                	srow += '<li><span class="text-info"><button onclick="searchKey(1,'+ menu.menu_seq + '); event.stopPropagation()" class="btn btn-mini btn-info pointer"> <i class="fa fa-folder-open"></i>하위메뉴 ('+ menu.childMenuCnt +')</button></span></li>';
                }
                srow += '</ul>';
                srow += '</div>';
                srow += '</td>';
...
function menuUpdate(e){
	...
	window.location.href = '/admin/adminMenuUpdate/' + e;
}
