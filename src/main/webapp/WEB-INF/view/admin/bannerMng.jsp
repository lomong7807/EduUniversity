				...
				<div id="container">
					<div class="dashboard">
						...
						<div class="row form-inline mb10">
							<div class="col-sm-6">
								<h3 class="text-info mt5">배너관리</h3>
							</div>
						</div>
						<!-- //페이지 명 -->
						<!-- 목록 -->
									...
									<form name="form">
											...
											<table id="data-table" class="table text-center table-hover">
												<thead>
													<tr class="headings">
														...
														<th class="column-title hidden-xs">배너 이미지</th>
														<th class="column-title hidden-xs">배너 문구</th>
														<th class="column-title hidden-xs">링크 URL</th>
														<th class="column-title hidden-xs">작성자</th>
													</tr>
												</thead>
												<!-- 테이블 정보 위치 -->
												<tbody>
										...
										<div class="row pull-right">
											<a href="#" class="btn btn-mini btn-default"
												onclick="deleteAdmin();"> 취소하기 </a> <a
												href="/admin/bannerReg" class="btn btn-mini btn-primary">
												등록하기 </a>
										</div>
									</form>
									<!-- //게시물 -->
...
//현재 열려있는 도메인 추출
        ...
        $.ajax({
            ...
            success : function( data ) {
                ...
                data.adminList.forEach(function(admin, index) {
                    let row = '<tr class="odd pointer" data-href="/admin/bannerReg?banner_seq=' + admin.banner_seq + '">';
                        row += '<td class="checkbox-cell"><input type="checkbox" name="banner_seq[' + admin.banner_seq + ']" value="' + admin.banner_seq + '" class="flat"></td>';
                        row += '<td class="hidden-xs"style="min-width:100px">' + ((data.totalRecords - (data.currentPage - 1) * data.pageSize) - index) + '</td>';
                        if(admin.banner_image_url == null){
                        	row += '<td class="hidden-xs"></td>';
						} else {
                        row += '<td class="hidden-xs"><img  style="max-width:200px" class="img" src="'+admin.banner_image_url+'" alt="image"></td>';
						}
                        row += '<td class="hidden-xs" style="min-width:200px">' + admin.banner_text + '</td>';
                        row += '<td class="hidden-xs">' + admin.banner_link_url + '</td>';
                        row += '<td class="hidden-xs" style="min-width:150px">' + admin.write_id + '</td>';
    ...
    function deleteAdmin(page = 1) {
        ...
        // 체크된 체크박스들을 선택
        let checkedCheckboxes = $("input[name^='banner_seq']:checked");
