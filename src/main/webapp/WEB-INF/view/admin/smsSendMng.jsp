...
<script>
	...
	function searchKey(page = 1){
		let inputArr = $('#sms_tab1.panel-body').find('input, select');
		let arr = ['startd','searchCate', 'keyword'];
		...
		$.ajax({
			...
			success : function( data ) {
				...
				if (data.userList.length === 0) {
					...
					$('#data-table tbody').html('<tr><td colspan="10" class="text-center">검색된 결과가 없습니다.</td></tr>'); // Show no results message
					$('#totalRecords').text(0);
					return;
				}
				...
				data.userList.forEach(function(user, index) {
					...
					let row = '<tr class="odd pointer" data-href="/admin/userMngRegister?userSeq=' + user.user_seq + '">';
						...
						row += '<td class="hidden-xs">' + user.user_hp + '</td>';
