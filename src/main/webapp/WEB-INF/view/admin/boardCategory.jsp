...
<head>
<meta charset="utf-8">
<title>Board Category Mng</title>
				...
				<div id="container">
					<div class="dashboard">
						...
						<div class="row form-inline mb10">
							<div class="col-sm-6">
								<h3 class="text-info mt5">게시판 카테고리 관리</h3>
							</div>
						</div>
						<!-- //페이지 명 -->
						<!-- 목록 -->
						<form Method="Post" id="teacher_list" name="teacher_list">
							...
							<fieldset>
												...
												<div class="row">
													...
													<div class="col-sm-3">
														<select id="searchCate" name="searchCate" class="form-control">
															...
															<c:forEach var="item" items="${boardConfig }">
																<option value="${item.config_seq }">${item.board_name }</option>
															</c:forEach>
														</select>
													</div>
													<div class="col-sm-2">
														<button type="button" class="btn btn-block btn-default" onclick="searchKey();">
								...
								<table class="table text-center table-hover">
									<thead class="hidden-xs">
										<tr>
											...
											<th scope="col" class="hidden-xs">카테고리명</th>
											<th scope="col" class="hidden-xs">사용여부</th>
										</tr>
									</thead>
									<tbody>
										<!-- Loop -->
										<!-- <tr>
											...
											<td class="hidden-xs num-webfont"><span class="text-info mr5"> 1 </span></td>
											<td class="hidden-xs hidden-sm"><span class="text-info mr5"> Test </span></td>
											<td class="xs-text-left"><span class="text-info mr5"> thacher3 </span></td>
											<td class="hidden-xs"><span class="text-info mr5"> test </span></td>
											<td class="hidden-xs"><span class="text-info mr5"> test </span></td>
											<td class="hidden-xs"><span class="text-info mr5"> tester </span></td>
											<td class="hidden-xs"><span class="text-info mr5"> 예 </span></td>
										</tr> -->
										<!-- //Loop -->
								...
								<div class="row pull-right">
									<a href="#" class="btn btn-mini btn-default" onclick="deleteCates();"> 삭제하기 </a> <a href="#" class="btn btn-mini btn-primary" onclick="javascript:location.href='/admin/boardCategoryReg';">
										등록하기 </a>
								</div>
								<!-- //버튼 -->
							</fieldset>
    ...
    $.ajax({
        ...
        success : function( data ) {
            ...
            data.list.forEach(function(ele, index) {
                ...
                let row = '<tr class="odd pointer" onclick="updateCategory('+ ele.category_seq+ ')">';
                    row += '<td class="checkbox-cell"><input type="checkbox" onclick="event.stopPropagation();" name="cate_seq[' + ele.category_seq + ']" value="' + ele.category_seq + '" class="flat"></td>';
                    row += '<td class="hidden-xs">' + ((data.totalRecords - (data.currentPage - 1) * data.pageSize) - index) + '</td>';
                    row += '<td class="hidden-xs">' + ele.config_title + '</td>';
                    row += '<td class="hidden-xs">' + ele.title + '</td>';
                    row += '<td class="hidden-xs">' + ele.is_open + '</td>';
...
function updateCategory(e){
	window.location.href="/admin/updateCate/" + e;
}
...
function deleteCates() {
    // 체크된 체크박스들을 선택
    let checkedCheckboxes = $("input[name^='cate_seq']:checked");
    ...
    let delCates = checkedValuesArr.join(',');
    if(delCates === '' || delCates === null || delCates.length == 0){
        alert('선택된 카테고리가 없습니다.');
        return;
    }
    let data = {'delCates' : delCates};
    $.ajax({
        type : "POST",
        ...
        success : function( data ) {
			if(data == 1){
				alert('삭제 성공');
				window.location.href = '/admin/boardCategory';				
			}else{
				alert('error');
				return;
			}
